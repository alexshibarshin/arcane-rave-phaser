...
describe('StageFlowCoordinator', () => {
  it('publishes the initial snapshot and build intro on stage initialization', () => {
    const runtime = createStageRuntime(makeStageConfig({ totalWaves: 2, initialCoins: StageFlowConfig.INITIAL_COINS }), undefined, undefined, () => 0);
  ...
  it('emits stage:return-to-lobby with stars on stage completion', () => {
    ...
    const runtime = createStageRuntime(makeStageConfig({ id: stageId, totalWaves: 1, initialCoins: StageFlowConfig.INITIAL_COINS }), undefined, undefined, () => 0);
    const coordination = createStageFlowCoordinationState();
    requestStageWaveStart(runtime);
    dispatchStageFlowIntent(runtime, coordination, {
      type: 'stage:combat-ended',
  ...
  it('emits stage:return-to-lobby with stars on defeat', () => {
    const stageId = 'stage-1';
    const runtime = createStageRuntime(makeStageConfig({ id: stageId, totalWaves: 2, initialCoins: StageFlowConfig.INITIAL_COINS }), undefined, undefined, () => 0);
    const coordination = createStageFlowCoordinationState();
    requestStageWaveStart(runtime);
