...
Нельзя писать тесты, которые:

- assert'ят конкретные numeric/string значения из `src/config/`, если смысл теста
  не в самом контракте конфигурации
- фиксируют текущие тайминги, радиусы, урон, цены, оффсеты, координаты, counts,
  цвета, id волн, составы пресетов и прочие тюнинговые данные только потому, что
  “сейчас так в конфиге”
- превращают тест в snapshot текущего balance/content-авторинга

Нужно писать тесты, которые:

- проверяют инварианты: ссылки валидны, значения положительные, порядок фаз
  соблюдён, диапазоны корректны, состояния переходят правильно
- проверяют поведение систем при изменении конфигов, а не совпадение с их
  текущими числами
- используют сравнения вида “увеличилось/уменьшилось/не пусто/валидная ссылка/
  применился эффект/сработал приоритет”, если точное число не является частью
  публичного контракта

Если при тюнинге баланса или layout-конфига тест начинает падать без поломки
логики, значит тест написан неправильно и его нужно ослабить или удалить.
